...
### Changed
- **The K-1 and ETN notices no longer state today's position.** Through v1.43.0 each closed with
  "None of them are in the position as of the last market day, but the logic can route into any of
  them on any rebalance", or named the tickers that were held. That clause is gone; what remains is
  "The logic can route into any of them on any rebalance, so this is a possibility to plan for
  rather than a position to check."

  **Removed for a stronger reason than matching the Assets section.** A K-1 is generated by holding
  the fund at ANY point in the tax year, so a strategy that routed into UVXY in March and is out of
  it today still produces one. The clause answered a question nobody was asking, and it answered it
  as reassurance, placed inside a warning. The half that was load-bearing is the half that survives.

### Notes
- **`held` is still carried per ticker in `data/strategy_extras.json` and now has no consumer.**
  Left in place deliberately, unlike the dead CSS removed in v1.43.0: `CARRY` in
  `build_strategy_extras.py` already holds several fields no page renders, and this is a true fact
  about the join rather than markup that can only mislead by surviving.
- **Verified by re-running the full 36 page headless Edge sweep** with "in the position as of the
  last market day" added to the list of phrases that must not appear. 36 of 36 pass, and the
  rendered notices were read by eye on a strategy with one affected ticker (`holy-grail`, UVXY),
  five (`four-horsemen`), and two ETNs (`gold-miner-original`). All four deploy gates pass.
