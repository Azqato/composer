...
```json
{
  ...
  "tickers": {
    "QQQ": { "name": "Invesco QQQ Trust", "closes": [156.5, 157.2, "..."] },
    "GDXU": { "name": "MicroSectors Gold Miners 3x Leveraged ETN", "closes": [null, null, "..."] }
  }
}
```
...
| `tickers` | object | Map of `symbol → { name, closes }` |
...
- [ ] **Signal Miner: live filter updates without a full re-run (requested 2026-08-15)** — today, changing the section-3 filters (Min Time in Market, Max Drawdown floor, Prune quantile, and the AND-pairing toggle) requires clicking "Run backtest" again, which re-does all the expensive per-signal backtesting. The ask: adjusting those filters should update the results in real time. Feasible because filtering is cheap relative to backtesting — the per-signal backtest metrics (`tim`, `maxDD`, `total`, `sortino`, `calmar`) do not change when a threshold changes; only which rows *pass* changes. Plan: after a run, cache the full unfiltered result set (every single-signal metric, and ideally the raw pairing inputs), then re-apply the TIM/MDD/quantile thresholds against that cache on `input`/`change` (debounced) to re-filter and re-rank instantly. The genuinely expensive part to handle is 2-signal AND pairing, which depends on the quantile-pruned survivor set — either recompute just the pairing pass on filter change (still far cheaper than Pass 1) or cache pair metrics too. Note this is distinct from the existing results-table text filter (`sl-filter`), which already filters client-side without re-running.
- [ ] **ETF Cloner: normalize share-class tickers to Composer/Crescendo format (reported 2026-08-15, community QA)** — holdings sources (stockanalysis.com live feed and issuer CSV/xlsx files) render multi-class tickers with a **dot**, e.g. `BRK.B`, `BF.B`, and some use a **dash**, `BRK-B`. Composer *and* Crescendo require the **slash** form `BRK/B`; a symphony containing `BRK.B` will not save and will not backtest on either platform (confirmed by community testers: swapping to `BRK/B` fixed it). Both notations are technically valid, it's purely a cross-platform compatibility issue. Fix: in the ETF Cloner, normalize the class separator to `/` before emitting the Composer JSON — convert a trailing `.X`/`-X` class suffix to `/X` (e.g. `BRK.B` → `BRK/B`, `BF.B` → `BF/B`, `BRK-B` → `BRK/B`) on **both** input paths (live fetch and file upload). Note `cleanSym()` currently keeps `.` and `-` in its allowed pattern (`/^[A-Z][A-Z.\-]{0,9}$/`) and passes them through unchanged; add the separator normalization there (or at symphony-build time) and re-QA against broad-market funds like VTI/VT that hold class shares. Care: only rewrite an actual class suffix, not a legitimate `.`/`-` that is part of a different symbology.
- [ ] **Full site-wide naming review of every tool/page (requested 2026-08-15)** — do one deliberate pass over the names of all public pages/tools for clarity, consistency, and a minimal, coherent design voice, rather than renaming piecemeal. Current set to review: **Home, Strategies, Database, RSI Signals, Signal Miner, Converter, ETF Cloner, Glossary, About**. Things to weigh: one-word vs. two-word names (user prefers one-word where it does not cost clarity), whether each bare name is self-explanatory to a first-time visitor, and cross-tool consistency (e.g. the "Signals / Miner / Cloner / Converter" family). Context from the 2026-08-15 discussion: "Signal Lab" was renamed to "Signal Miner" (v1.16.6) for accuracy; shortening to just "Miner" was considered and rejected because on an investing site it reads as crypto/mining-stocks without the "Signal" qualifier. Any rename must carry through all surfaces at once (page `<title>`, nav Tools dropdown + footer + homepage Explore card in `js/app.js`/`index.html`, in-page hero/copy, and a `noindex` redirect stub from the old URL so links do not break, per the Signal Miner rename pattern).
