...
| High | 200 | 5.8ms | 8.7ms | 8.7ms | 40% | ~40% |
| **Medium (default)** | 150 | 4.4ms | 17.4ms | 17.4ms | 20% | ~20% |
...
**Filter defaults.** Min Time in Market `0.05`, Max Drawdown floor `-0.8` (both v1.16.4), prune quantile `0` (v1.16.8, so nothing is trimmed before pairing and all qualifying signals show by default), combo depth `2`, min period `10`. Two buttons in the Target tickers row restore a whole preset: **Default** (v1.22.4) calls `loadDefaults()`, which puts back `DEFAULT_TARGETS` / `DEFAULT_COMPARES` and every filter above, and **Community V2 Example** calls `loadExample()`, which loads the 8-ticker heavy run and deliberately drops min period to `5`. These three places (the init defaults, `loadDefaults`, and `loadExample`) each hold their own copy of the values; keep them in sync when changing any filter default. Both loaders null `sigCache`, since changing the ticker set makes the previous run's cached results inapplicable to the live filters. Prune quantile applies within each target ticker and runs **sequentially** across total return, profit factor, Sortino, and Calmar (matching the source notebook), so its effect compounds: `0.5` does not leave half the signals, it leaves what survives the top half on each of the four metrics in turn.
...
**Next up on the Signal Miner, in the order agreed 2026-08-20.** Items 1 and 2 are the explicit remainder of the v1.23 plan; the rest were surfaced by it.

- [ ] **1. OR combining, and Composer's any/all multi-ticker conditions.** Asked for directly by the owner (2026-08-20, pointing at Composer's `+` button on a condition row). This is the one piece of the agreed v1.23 rollout that did **not** ship, because the instruction list skipped it. Two genuinely different features got conflated in conversation and must be kept apart:

  **(a) OR between two conditions.** Combining is AND-only today: `applyFilters` pairs survivors and emits a row whose booleans are `a[i] && b[i]`. Composer's `compound` condition already takes `operator: 'all'` or `'any'`, and `composerThenChild` hardcodes `'all'`. The work: a **Combine mode** control in section 3 (AND / OR / both), an OR variant in the pairing loop, `operator: 'any'` in the export when the row is an OR pair, and a mode column (or a suffix on the label) so the table never shows two rows that read identically but mean opposite things. Cheap: the survivor set is unchanged and the pairing loop just emits a second row per pair, so "both" roughly doubles the pairing pass, which is already the cheap half of a run.

    **Design caution to settle before building.** An OR pair's Time in Market is at least as high as either single, so OR rows drift toward buy-and-hold exactly where AND rows drift toward over-fitting. The existing Min Time in Market floor screens the wrong direction for them. Expect OR results to need either a separate ranking, a Max Time in Market ceiling, or at minimum a note; do not just merge them into the same leaderboard and call it done.

  **(b) any/all across a ticker list inside ONE condition.** This is what the `+` button actually does, and it is not OR-of-two-conditions. The schema is already in the exporter: a `binary-compound` carries `operator: 'any' | 'all'` and a `tickers: []` array, and `composerCondition` already emits `operator: 'any'` with a single-element array. Mining it means a spec whose left operand is a *set* of tickers, e.g. "RSI(10) of **any of** [TQQQ, SOXL, TECL] < 30".
    ...
    **This is combinatorially explosive if left unbounded** and must not be built as "all subsets": 72 tickers is 2^72 sets. Bounded designs worth costing, cheapest first:
    1. **Asset-class groups as the candidate sets** (Leverage, Inverse, Bonds, Value & Dividend, and the rest, straight off the `group` field in `prices.json`). Nine-ish sets instead of 2^72, each with any/all and both directions. Recommended starting point: it is bounded, meaningful, and needs no new UI beyond a checkbox.
    2. The user's whole selected Signal set as one group, which is one extra set per family and effectively free.
    3. Unordered pairs and triples of selected tickers, which is `C(n,2) + C(n,3)` and is already 62,196 sets at 72 tickers before any family multiplier. Only viable on a small selection, and it needs its own guard.

    `evalSpec` needs a set-valued left operand (fold `any`/`all` across the member series before comparing), the label needs to name the set, and the spec store needs a set index rather than a single ticker byte. Budget this as the larger of the two halves.

- [ ] **2. Recalibrate the estimate warn/stop thresholds.** `SIGNAL_WARN_CAP` (100,000, triggers the run confirm) and the 45,000 `warn` styling were set when a default eight-ticker run was around 20,000 signals. After v1.23.1 the same eight-ticker run is **100,696**, so an entirely ordinary selection now trips the "this is a very large batch" dialog on every run. That is a false alarm and it will train people to click through the one warning that matters. Either raise both numbers to match the new search space, or make them relative to something (rows retained, or projected wall time, which is what the user actually cares about). Do not simply delete them: the memory ceiling they guard is real. Small, and it should be done before anyone is asked to use the new grid in anger.

- [ ] **3. Skip cross-ticker operand pairs for the price-scale families.** `map_cmp`, `ema_cmp` and `stdp_cmp` compare **dollar magnitudes** across tickers. "MA-Price(42d) of SPY < MA-Price(42d) of QQQ" is not a signal, it is a statement about share prices, and it was measured constant-false over the whole sample during v1.23.0 verification. Those specs cost full price and self-prune only at the end of Pass 1, where a constant-false signal produces no trades and fails the `total > 0` gate.

  The arithmetic is stark. At 72 tickers with the 10-day floor, `map_cmp` is **875,160 specs**, of which the same-ticker fast-versus-slow pairs (the golden cross, the entire reason the family exists) are only **11,232**. So roughly **98.7% of the family is dollar comparisons**. Restricting the three price-scale families to same-ticker operand pairs would cut about **864,000 specs from a default run, near 18% of the total**, with essentially no loss of real coverage.

  **Measure before cutting.** Count what fraction of cross-ticker price specs are actually degenerate (more than 99% or less than 1% true across the sample) rather than assuming it. Two funds with similar share prices genuinely do cross. The likely outcome is a per-family `sameTickerOnly` flag rather than a blanket rule, since `rsi_cmp`, `cum_cmp`, `ma_cmp` and `std_cmp` are all scale-free and must keep their cross-ticker pairs.

- [ ] **4. Re-measure the memory ceiling on the new search space.** The v1.22.15 profile (527 MB peak heap, 1,502 MB peak process) was taken on the 3,042,720-signal run. The default maximal run is now **4,800,024**, 58% larger, and specs went columnar in v1.23.0 which should more than absorb it. Re-run the `harness-scale.py` A/B to confirm the crash point moved where predicted rather than assuming it did, and update the figures in Section 12. Remember the constraint that makes this matter: a tab's heap ceiling is fixed at roughly 3.5GB and **does not grow with installed RAM** (measured 3,586 MB on a 48GB machine), so this is about the tab, not the user's hardware.

- [ ] **5. Persist the headless Edge harnesses into `scripts/`.** Four exist and all four live only in the session scratchpad, so every session rebuilds them: behavioural A/B (`runeq.py` + `eq.js`, drives HEAD and the working tree through an identical scripted session and byte-diffs the output), snapshot round trip (`runsnap.py` + `snap.js`, two Edge invocations sharing one `--user-data-dir` so `localStorage` survives), memory A/B (`run_scale.py` + `harness-scale.js`), and the grid/family verifier (`grid.js`). They are the only reason any of the v1.22 and v1.23 claims are measured rather than asserted, and losing them means the next change is verified by eye.

  **Four traps to carry across with them, all learned the hard way:** the page's entire script is wrapped in an IIFE, so a harness must be handed internals through a `window.__t = { ... }` hook spliced in before the trailing `})();` rather than injected as a sibling `<script>`; `window.confirm` auto-dismisses under `--headless=new` and must be stubbed to `() => true`; the CPU throttle feeds back on itself under `--virtual-time-budget` and must be neutralised **in the test copy only** (`cpu === 'max' ? 0.25` to `? 0`), identically for both sides of any A/B; and the dumped DOM must be read as **bytes** then decoded UTF-8, because Python will otherwise decode it as the console codepage and die on the first non-ASCII byte. Also: **use Edge, never Chrome**, so the owner's browser session is untouched.

- [ ] **6. `scripts/check_live.py`: lint what Cloudflare actually serves.** Cloudflare Pages deploys independently of the GitHub Actions gate and the build command cannot be changed, so "pushed" and "live" are genuinely different states. This was checked by hand twice on 2026-08-20 (fetch `https://composeratlas.com/signal-miner`, compare byte-for-byte against the committed file, then grep a list of markers that should be present and a list that should be absent). Worth twenty lines and a place in the deploy runbook.

- [ ] **7. `.assetsignore` so Cloudflare stops serving `docs/`, `scripts/` and `README.md`.** They are published today as a side effect of deploying the repo root. No secrets are involved, but `docs/PRD.md` is an internal planning document and there is no reason for it to be a public URL.

- [ ] **8. Report *measured* CPU duty rather than the nominal figure.** Long-standing, and sharper now. Only the Max setting is distorted by the `setTimeout` clamp (it displays 80% and delivers about 64%), and the distortion always errs toward *less* CPU. Reporting the measured busy fraction in the status line would close the gap without touching the throttle. **Do not close it by raising the ceilings**: they are capped for heat, reaffirmed by the owner on 2026-08-20 after the memory fix, and the tool runs on visitors' hardware.

- [ ] **9. Indicator warm-up is still not added to the sample-window start.** Pre-existing, and the standardized grid makes it bite harder: every family now reaches 252 days, so a 252-day signal on the binding ticker silently loses its first 252 days inside the common window. Making the window start per-spec was judged not worth the complexity when it was a two-family problem; it is now a sixteen-family problem and deserves a second look, or at minimum a note in the results meta line.
