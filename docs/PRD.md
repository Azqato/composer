...
- [~] **A. Parameter plateau scoring: rank on the neighbourhood, not on the peak.** Owner's idea, 2026-08-21. **The diagnostic shipped 2026-09-03 in v1.73.0; the re-ranking did not, and is the only part still open.** See the closing subsection below for what was built, what the first real run measured, and where the design departed from the reasoning here. Everything from this point to that subsection is the original 2026-08-21 argument, kept as written. A row's rank says nothing about whether the parameter it found is a knife edge. Suppose the leaderboard shows `RSI(10) of SMH < 23` at 2.8 Calmar. If `< 22` and `< 24` score 0.9 and 1.1, that 23 is a spike: the rule works at exactly one threshold and nowhere near it, which is what over-fitting looks like from the inside. If instead the whole span from 20 to 28 scores between 2.4 and 2.9, that is a **plateau**, the node fires usefully across a range of values, and the result is far more likely to be a real effect than an artifact of where the lattice happened to land.
  ...
  **What the first real run measured**, top 100 single signals, QQQ target against SPY/QQQ/TLT, min period 12:

  | Plateau width (steps) | 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
  |---|---|---|---|---|---|---|---|---|
  | Rows | **18** | 25 | 19 | 17 | 15 | 3 | 1 | 2 |

  The neighbourhood median runs at **0.91** of the row's own Calmar at the median and **0.73** at the tenth percentile. **Eighteen of the top hundred hold at exactly one setting and nowhere adjacent**, which is what over-fitting looks like from the inside, and until this shipped they were indistinguishable from the other eighty-two.
  ...
  **Verified by `scripts/run_harness.py plateau`,** a fourth headless gate. It recomputes every plateau independently rather than re-reading the page's own numbers: 394 axis ends and 415 in-band steps checked against its own boolean pass and backtest, and 100 neighbourhood medians reproduced exactly. Two things that could only have been caught that way: the driver's first version rested its entire single-row sample on **one** row, because at the shipped pairing setting the top 100 is 99 combined rows; and two independently derived accounts of the same 394 axis ends disagreed by exactly 20, which turned out to be the page saying a plateau "may run wider" in the one case where the grid ended a step later. Both are recorded in `scripts/harness/README.md`.
