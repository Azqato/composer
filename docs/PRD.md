...
**Every level was lowered at v1.22.11, and Max stopped meaning 100%** (owner report: it was crashing browsers). Previously Max ran with `FACTOR` 0, no idle at all, on a batch of 500 while every other level used 150. Both halves of that were harmful. Uncapped duty is the obvious one; the batch size is the subtler one, because **the batch boundary is the only point the loop yields**, so 500 held the main thread for over three times as long between breaths, and a tab that never yields long enough is a tab the browser can kill. Batch size is now a uniform **150** at every level, and `FACTOR` is derived from the target duty as `100/duty - 1`, verified to round-trip: 0.25 to 80%, 1.5 to 40%, 4 to 20%, 9 to 10%. Since no level is now unthrottled, the old unreachable "max speed" status branch and the `FACTOR ? ... : 0` sleep guard were both removed rather than left as dead code.

Option values are `max`/`high`/`medium`/`low`, renamed from `full`/`balanced`/`eco`/`cool` at v1.16.10 specifically because the old `eco` value meant the ~25% tier and would have collided with an "Eco" label on the ~10% tier. The status line shows a live estimate of **time remaining** (v1.18.1 added it; v1.22.10 trimmed the projected total and idle-time figures that used to sit beside it, since the CPU percentage already implies why a throttled run outlasts its compute time). The projection itself is unchanged and still drives the remaining figure: total and idle are projected from *compute* measured so far, scaled by the duty factor, and remaining is `projTotal - elapsed` off the wall clock, so the two always agree and remaining counts down monotonically. Deriving total from wall-clock elapsed instead was tried and rejected: at a batch boundary the current batch's sleep has not happened yet, so `elapsed / frac` understates the total by the entire `1 + FACTOR` on the first reading (5x on Medium, 10x on Low) and then climbs, which reads as a broken estimate. This only governs speed and heat; results are identical at every setting.
